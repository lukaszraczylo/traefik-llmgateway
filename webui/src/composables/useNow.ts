...
export interface UseNow {
  /** The shared clock — every time-dependent read on the Usage tab (CostForecastCard's elapsedFraction, usageColumns()'s own `now` parameter, UsageView.vue's groupCostProjection) reads THIS ref, never its own `new Date()`. */
  now: Ref<Date>
...
/**
 * useNow (P1) is the ONE reactive clock every time-dependent read on the
 * Usage tab shares, instead of each freezing its own `new Date()` at mount
 * or re-deriving a fresh one on every render:
 *
 ...
 *   usageColumns(...))` and UsageView.vue's headless Groups toolbar
 *   columns both called it with no reactive dependency on the current
 *   time, so neither ever recomputed either.
 * - UsageView.vue's groupCostProjection called `new Date()` fresh on every
 *   invocation instead, so the group grid could disagree with the above
 *   two on what "now" even was, mid-render.
 *
 ...
 *
 * This composable does NOT call onMounted/onUnmounted itself — the caller
 * (UsageView.vue) owns its lifecycle explicitly via the returned start/stop,
 * the same reason lib/polling.ts itself takes no lifecycle-hook dependency:
 * it keeps this composable callable, and testable, outside a mounted
 * component.
 */
