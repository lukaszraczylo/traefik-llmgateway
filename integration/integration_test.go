...
// reflects the REAL LLM traffic this suite has generated by the time this
// test runs — deliberately placed near the end of the file (Go runs
// tests in source order with no -shuffle in this suite's Makefile
// target) so alice's own request counters are already non-zero. It also
// asserts the stats-exclusion directive's other half: the admin key
// itself, used throughout this suite only to call /admin/api/* routes,
// must show exactly 0 requestsPerDay — admin traffic is never counted.
func TestAdminDashboard(t *testing.T) {
	...
	if reqDay, _ := aliceUsage["requestsPerDay"].(float64); reqDay <= 0 {
		t.Errorf("alice requestsPerDay = %v, want > 0 (this suite's own earlier real-LLM traffic)", reqDay)
	}
	// Stats-exclusion directive (progress ledger, 2026-08-21): admin routes
	// never call checkAndCount, so the admin key — used throughout this
	// suite ONLY to call /admin/api/* (adminUsageEntry/adminUsageHistory,
	// plus this test's own overview/usage calls above) — must show exactly
	// 0 requestsPerDay despite having issued many requests by this point in
	// the run. This inverts the pre-directive assertion here, which
	// expected admin's own admin-API calls to count like any other route.
	adminUsage := adminUsageEntry(t, "users", "admin")
	if reqDay, _ := adminUsage["requestsPerDay"].(float64); reqDay != 0 {
		t.Errorf("admin requestsPerDay = %v, want 0 (admin routes never call checkAndCount)", reqDay)
	}
